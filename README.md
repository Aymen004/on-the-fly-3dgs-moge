...
<p align="center">
  <img src="https://github.com/user-attachments/assets/1c7dd79a-d525-4d29-8993-4e41062cf999" alt="image 1" width="50%" />
...
- +1.17 dB mean PSNR on held‑out views
- Statistically significant SSIM/LPIPS improvements

How it works:
- Single‑image → dense 3DGS: Run MoGE‑2 on one image, convert to a 3DGS‑compatible PLY, and place it at `results/<model>/point_cloud/point_cloud.ply` before training.
- Minimal hook: At startup, training checks for that file; if present, it loads the cloud and skips the geometric bootstrap entirely.
- Incremental loop unchanged: Pose tracking, LoG sampling, and joint optimization proceed as in the original method.

Why it matters:
- Dense prior at t=0 (vs. sparse SfM) reduces early floaters/noise and stabilizes tracking.
- No bootstrap stage → the incremental loop starts immediately (less compute, faster convergence).
- Stronger optimization trajectory thanks to a credible learned prior.

Technical summary (PLY fields we write):
- Positions: `(x, y, z)` from MoGE‑2
- Color (SH‑DC): `f_dc = (color_linear − 0.5) / C0`, with `C0 = 0.28209479177387814`
- Opacity: logit domain (e.g., `logit(0.99) ≈ 4.5951`)
- Scale: per‑axis log‑space (e.g., `log(0.01)`)
- Rotation: unit quaternion `[1, 0, 0, 0]`
- Normals: carried through but unused by 3DGS rasterization

Hook concept :
- The original `on-the-fly-nvs` already persists and reloads the active Gaussian scene at `results/<model>/point_cloud/point_cloud.ply`.
- We leverage this invariant: pre-populate that path with our MoGE-generated PLY before calling `train.py`.
- At startup, the repository naturally loads this file and therefore skips the geometric bootstrap over images.
- No changes to pose tracking, LoG sampling, optimizer, data readers, or renderer—just a tiny file-existence check.
- Benefits: zero fork, fully reversible, easy to upstream or toggle, and perfectly compatible with the upstream training loop.
Minimal hook (pseudocode):
```python
cloud = Path(results_dir) / 'point_cloud' / 'point_cloud.ply'
if cloud.exists():
  scene = load_gaussian_cloud(cloud)   # start from MoGE prior
else:
  scene = run_geometric_bootstrap()    # original bootstrap
```
...
Our method improves 66.7% of test views; the right tail reaches +16.34 dB in the hardest cases (low parallax, low texture) where the original bootstrap is fragile.
